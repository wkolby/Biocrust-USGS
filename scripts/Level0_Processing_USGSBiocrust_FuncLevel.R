...
ggplot(dataset_mean_std, aes(x=wavelength,y=mean,group=type,color=type)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(text = element_text(size = 18))
ggsave(paste(github_dir,'/figures/Line_Headwall_Hyperspectra_B2_VNIR_ByFunc_FuncLevel.png',sep=''),dpi=300,width=180,height=120,units='mm')
...
#################################################################################
#######UAS RESONON##############################################################
master <- read.csv2(paste(github_dir,'/data/Resonon/Resonon_Datasheet_021422_021522.csv',sep=''),sep=',',header=T)
dataset <- data.frame()
for(i in 1:length(master$File)){
  temp_data<-read.table(paste(github_dir,'/data/Resonon/',master$Dir[i],'/',master$File[i],sep=''),skip=3)
  data_c<-as.numeric(temp_data[,2])
  wvl=as.numeric(temp_data[,1])
  data_s=data_c/sqrt(sum(data_c^2))
  plot=rep(master$Plot[i],length(wvl))
...
ggplot(dataset_mean_std, aes(x=wavelength,y=mean,group=type,color=type)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(text = element_text(size = 18))
ggsave(paste(github_dir,'/figures/Line_Resonon_Hyperspectra_B2_VNIR_ByFunc_FuncLevel.png',sep=''),dpi=300,width=180,height=120,units='mm')
...
out<-cbind(df_wide$full,master$Plot,master$Treat,master$Biocrust,master$Rep,df_wide$NDVI,df_wide$CI1,df_wide$CI2)
colnames(out)<-c('ID','Plot','Treat','Type','Rep','NDVI','CI1','CI2')
write.csv(out,paste(github_dir,'/data/Level1/Resonon_Chlorophyll_Indices_Full.csv',sep=''),row.names=FALSE,col.names=TRUE)
