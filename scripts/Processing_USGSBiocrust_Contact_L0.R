...
ggplot(subset(datasetB2_mean_std, treat %in% c('CC','LW')), aes(x=wavelength,y=mean,group=type,color=type)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(text = element_text(size = 18))
ggsave(paste(github_dir,'/figures/Hyperspectral_Reflectance_B2_VNIR_ASD.png',sep=''),dpi=300,width=180,height=120,units='mm')
...
ggplot(subset(datasetB3_mean_std, treat %in% c('CC','LW','LC','CW')), aes(x=wavelength,y=mean,group=type,color=type)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(text = element_text(size = 18))
ggsave(paste(github_dir,'/figures/Hyperspectral_Reflectance_B3_VNIR_ASD.png',sep=''),dpi=300,width=180,height=120,units='mm')
