...
ggplot(dataset_mean_std, aes(x=wavelength,y=mean,group=treat,color=treat)) +
  ...
  #scale_fill_manual(values=c('cyan2','green3','red3','purple'))+
  #facet_wrap(~plot)+
  scale_y_continuous("Reflectance",limits = c(0,0.3), breaks = seq(0,0.4,0.1)) +
  scale_x_continuous("Wavelength (nm)",limits = c(400,900), breaks = seq(400,900,100)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(text = element_text(size = 18))
ggsave(paste(github_dir,'/figures/Line_Resonon_Hyperspectra_VNIR_PlotLevel.png',sep=''),dpi=300,width=180,height=120,units='mm')
...
ggplot(dataset, aes(x=wavelength,y=reflectance,group=treat,color=treat)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(text = element_text(size = 18))
ggsave(paste(github_dir,'/figures/Line_Resonon_Hyperspectra_VNIR_byTreat_PlotLevel.png',sep=''),dpi=300,width=180,height=120,units='mm')
